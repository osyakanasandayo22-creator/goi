...
let recentTopicsForAvoidance = [];
let sessionGeneratedTopics = [];
...
function setLoading(loading, message = "") {
  ...
  if (statusText) {
    ...
    // 完了メッセージなどはしばらくしたら自動で消す
    if (!loading && message) {
      const currentMessage = message;
      setTimeout(() => {
        if (statusText.textContent === currentMessage) {
...
function setTopicLoading(loading, message = "") {
  if (topicStatusText) {
    topicStatusText.textContent = message;
  }
  if (randomTopicButton) {
    randomTopicButton.disabled = loading;
  }
  // お題生成中は誤タップ防止で実行ボタンもロック
  if (runButton) {
    runButton.disabled = loading;
  }
}
...
async function loadHistory() {
  ...
  try {
    ...
    // 直近のトピック一覧をAIお題生成用に保持（重複をざっくり避けるため）
    recentTopicsForAvoidance = Array.from(
      new Set(
        snap.docs
...
randomTopicButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const allAvoidTopics = [
      ...recentTopicsForAvoidance,
      ...sessionGeneratedTopics,
    ]
      .map((t) => t.trim())
      .filter((t) => t.length > 0);
    ...
    if (allAvoidTopics.length > 0) {
      ...
      avoidanceBlock =
        "\n【最近出題されたお題の例】\n- " +
        uniqueShort.join("\n- ") +
        "\n\nこれらとほとんど同じ内容や、明らかに言い換えただけのお題は避けてください。";
    }
...
説明や前置き、コードブロック記号（バッククォート など）は書かないでください。
    ...
    if (topic.length > 0) {
      sessionGeneratedTopics.push(topic);
    }
    ...
    setTopicMode("ai");
    setTopicLoading(false, "お題をセットしました。");
  } catch (e) {
    console.error(e);
    setTopicLoading(false, "お題生成でエラーが発生しました。");
